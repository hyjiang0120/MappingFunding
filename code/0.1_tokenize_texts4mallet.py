...
python 0.1_tokenize_texts4mallet.py 0.1.3_abstractsinyear_path.txt ../data/clean_data/UK/fine_scale/splitdata

"""

__appname__ = 'TokenizeTexts'
__author__ = 'Flavia C. Bellotto-Trigo (flaviacbtrigo@gmail.com), Hanyi Jiang (hj22@ic.ac.uk)'
__version__ = '0.0.1'
...
class TokenizeTexts:
    ...
    def tokenize_csv(self):
        ...
        for fname in self.dirnames:
            ...
            with pd.read_csv(fname.split()[0], chunksize=1000, encoding='latin-1') as f:
                # print(len(f))
...
def main(argv):
    #read in stop words
    with open("0.1.2_stop-words.txt","r") as f:
    ...
    tokenize_text = TokenizeTexts(text_processor=text_processor, dirnames=dirnames, savefile=os.path.join(argv[2], "2022_2024_titles-abstracts-tokenized.csv"))
